...
namespace nadeemse\moduleMigration;
...
class ModuleMigrateController extends MigrateController
{
    ...
    protected function attachModuleMigrations()
    {
        ...
        $directories = glob('api/modules' . '/*' , GLOB_ONLYDIR);

        foreach ($directories as $basePath) {

            $nameArray = explode('api/modules/', $basePath);

            $path = $basePath . DIRECTORY_SEPARATOR. 'migrations';

            if (file_exists($path) && !is_file($path)) {
                $this->allMigrationPaths[$nameArray[1]] = $path;
            }
        }
    }
